...
class GameWindow:
    ...
    def load_components(self):
        ...
        def update_points(pu=0, ps=0):
            ...
            user_frame = tk.Frame(self.frame,highlightbackground="blue", highlightthickness=1)
            points_user_label = tk.Label(user_frame, text= user_name +' => ' +  str(self.points_user))
            points_user_label.pack()
            user_frame.grid(row=7,column=points_col_span,columnspan=points_users_col_span)

            system_frame = tk.Frame(self.frame, highlightbackground="blue", highlightthickness=1)
            points_system_label = tk.Label(system_frame, text='COMPUTER' + ' => '+ str(self.points_system))
            points_system_label.pack()
            system_frame.grid(row=7,column=points_col_span+points_users_col_span,columnspan=points_users_col_span)
            

            configure_bg(user_frame,system_frame,points_system_label,points_user_label)
        ...
        def show_chosen_system(k):
            ...
            user_option.grid(row=1,column=options_col_span+chosen_col_span+vs_col_span,rowspan=6,columnspan=chosen_col_span,padx=20)
        ...
        for i in range(len(Data.actions.items())):
            ...
            option_user.grid(row=1 + i, column=options_col_span+option_size*2+vs_col_span, columnspan=options_col_span,pady=10)
        ...
        def show_chosen_user(k,ignore=False):
            ...
            user_option = tk.Label(self.frame, image=img)
            user_option.image = img
            user_option.grid(row=1,column=options_col_span,rowspan=6,columnspan=chosen_col_span,padx=20)
            configure_bg(user_option)
...
class WinnerWindow:
    ...
    def load_components(self):
        player_frame = tk.Frame(self.frame, highlightbackground="blue", highlightthickness=1)
        player_name = tk.Label(player_frame, text=user_name + ' Score: '+str(self.score).zfill(2),font=('Arial', 30))
        player_name.pack(padx=10)
        player_frame.pack()
        configure_bg(player_frame,player_name)

        exit_btn = tk.Button(self.frame, text='EXIT',font=('Arial', 20),command=lambda _:self.exit_game())
        exit_btn.pack(padx=10,side=tk.LEFT,pady=5)
        configure_bg(exit_btn)

        restart_btn = tk.Button(self.frame, text='START NEW\nGAME!',font=('Arial', 14),command=lambda _:self.restart_game())
